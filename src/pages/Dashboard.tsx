...
const Dashboard: React.FC = () => {
  ...
  const { user, isGuest, isProfileLoading, isAppRefreshing, profileVersion } = useUser()
  ...
  if (isProfileLoading || isAppRefreshing) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-black text-white">
        <div className="text-2xl font-bold animate-pulse">Loading profile...</div>
      </div>
    )
  }
          ...
          {isGuest ? (
            <>
              <div className="text-center mb-12">
                ...
                <p className="text-gray-400 text-lg">You have limited access. Upgrade to unlock all features.</p>
                <button
                  className="mt-6 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold shadow"
              ...
              <div className="grid md:grid-cols-2 gap-8">
                {dashboardFeatures.map((feature) => (
                  <FeatureCard
                    ...
                    icon={feature.icon}
                    locked={!feature.allowGuest}
                    lockReason={!feature.allowGuest ? 'Sign up to unlock this feature' : undefined}
                    onClick={() => {
                      if (feature.allowGuest) navigate(feature.path)
                    }}
                  />
                ))}
              </div>
