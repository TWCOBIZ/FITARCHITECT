...
app.get('/api/meal-plans', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
...
app.post('/api/meal-plans/generate', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const generatedPlan = await openaiService_1.openaiService.generateMealPlan(userProfile, {
        ...
        const savedPlan = await prisma_1.prisma.mealPlan.create({
            data: {
                userId: user.id,
                dietType: dietType || 'balanced',
                meals: generatedPlan,
...
app.delete('/api/meal-plans/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        // Verify ownership and delete
        const deletedPlan = await prisma_1.prisma.mealPlan.deleteMany({
            where: {
                id: parseInt(id, 10),
                userId: user.id
            }
