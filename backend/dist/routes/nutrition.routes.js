...
router.get('/', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info(`Successfully fetched ${logs.length} nutrition logs for user ${userId}`);
    res.json(logs);
}));
...
router.post('/', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!date || !foods || !Array.isArray(foods)) {
        throw new validation_1.AppError(400, 'Invalid input. Date and foods array required.');
    }
    // Validate food entries structure
    for (const food of foods) {
        if (!food.name || typeof food.calories !== 'number') {
            throw new validation_1.AppError(400, 'Invalid food entry. Each entry must have name and calories.');
        }
    }
...
router.put('/:id', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!existingLog) {
        throw new validation_1.AppError(404, 'Nutrition log not found');
    }
...
router.get('/daily/:date', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
    const { date } = req.params;
    // Validate date format
    const dateObj = new Date(date);
    if (isNaN(dateObj.getTime())) {
        throw new validation_1.AppError(400, 'Invalid date format');
    }
...
router.get('/weekly', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
...
router.get('/monthly', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
...
router.get('/insights', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.id;
