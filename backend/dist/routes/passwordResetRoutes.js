...
router.post('/request', [
    ...
    try {
        ...
        // Send email
        const emailSent = await emailService_1.EmailService.sendPasswordResetEmail({
            email: user.email,
            resetLink,
            userName: user.name,
            expiresInMinutes: 15
        ...
        if (!emailSent) {
            ...
            logger_1.logger.warn('Email sending failed, logging reset link for development', {
        ...
        logger_1.logger.error('Password reset request failed', {
...
router.get('/verify/:token', async (req, res) => {
    try {
        ...
        logger_1.logger.info('Password reset token verified', {
        ...
        logger_1.logger.error('Password reset token verification failed', {
...
router.post('/reset', [
    ...
    try {
        ...
        if (resetData.attempts > 3) {
            resetTokens.delete(token);
            logger_1.logger.warn('Too many password reset attempts', {
        ...
        if (!user) {
            resetTokens.delete(token);
            logger_1.logger.error('User not found during password reset', {
        ...
        // Remove used token
        resetTokens.delete(token);
        logger_1.logger.info('Password reset completed successfully', {
