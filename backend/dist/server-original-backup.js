...
const TEST_USER_EMAILS = ((_a = process.env.TEST_USER_EMAILS) === null || _a === void 0 ? void 0 : _a.split(',')) || [];
const isTestUser = (email) => {
    if (!email || TEST_USER_EMAILS.length === 0)
        return false;
...
const setCache = (key, data, ttlMinutes = 10) => {
    cacheService_1.cacheService.set(key, data, undefined, ttlMinutes);
};
    ...
    try {
        ...
        try {
            healthCheck.services.openai = {
                ...
                usage: openaiService_1.openaiService.getUsageStats()
            };
        }
        catch (error) {
            healthCheck.services.openai = {
        ...
        try {
            const cacheStats = (0, cache_1.getCacheStats)();
...
app.get('/api/admin/workout-metrics', auth_1.authenticate, auth_1.requireAdmin, (req, res) => {
    try {
        const cacheStats = (0, cache_1.getCacheStats)();
        const openaiStats = openaiService_1.openaiService.getUsageStats();
        const metrics = {
            cache: cacheStats,
            openai: openaiStats,
            timestamp: new Date().toISOString(),
...
app.get('/api/diagnostics', auth_1.authenticate, async (req, res) => {
    var _a, _b, _c, _d, _e;
    try {
        const diagnostics = {
            timestamp: new Date().toISOString(),
            ...
            user: {
                ...
                tier: (_c = req.user) === null || _c === void 0 ? void 0 : _c.tier,
        ...
        try {
            const exerciseCount = await prisma_1.prisma.exercise.count();
            const activeExercises = await prisma_1.prisma.exercise.count({ where: { isActive: true } });
        ...
        // Check workout plans
        try {
            const planCount = await prisma_1.prisma.workoutPlan.count();
            const userPlans = await prisma_1.prisma.workoutPlan.count({
...
app.get('/api/diagnostic/exercise-sources', async (req, res) => {
    try {
        const results = {
            timestamp: new Date().toISOString(),
            tests: {},
        ...
        try {
            ...
            const sampleExercises = await prisma_1.prisma.exercise.findMany({
                where: { isActive: true },
                take: 10
            });
            const templateTest = {
...
app.get('/api/diagnostic/apis', async (req, res) => {
    var _a;
    try {
        const testResults = {
            timestamp: new Date().toISOString(),
        ...
        console.log(`🧪 API Tests - ExerciseDB: ${testResults.exerciseDbTest.status}, WGER: ${(_a = testResults.wgerTest) === null || _a === void 0 ? void 0 : _a.status}`);
...
app.use(loggingMiddleware_1.healthCheckMiddleware);
...
app.post('/api/upload-avatar', auth_1.authenticate, cloudinaryService_1.upload.single('avatar'), async (req, res) => {
    ...
    try {
        ...
        const currentUser = await prisma_1.prisma.userProfile.findUnique({
            where: { id: userId },
            select: { avatar: true }
        });
        ...
        if (error instanceof Error) {
            if ((_b = error.message) === null || _b === void 0 ? void 0 : _b.includes('cloudinary')) {
                errorMessage = 'Image service temporarily unavailable. Please try again later.';
            }
            else if ((_c = error.message) === null || _c === void 0 ? void 0 : _c.includes('prisma')) {
...
app.get('/api/trial-status', auth_1.authenticate, async (req, res) => {
    var _a, _b, _c;
    try {
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
        if (!userId) {
        ...
        const trialStatus = (0, auth_1.getTrialStatus)(req.user || null);
        res.json({
            isEligible: trialStatus.isEligible,
            isActive: trialStatus.isActive,
            daysRemaining: trialStatus.daysRemaining,
            hasUsed: trialStatus.hasUsed,
            userTier: ((_b = req.user) === null || _b === void 0 ? void 0 : _b.tier) || 'free',
            parqCompleted: ((_c = req.user) === null || _c === void 0 ? void 0 : _c.parqCompleted) || false
        });
    }
    catch (error) {
...
const subscriptionValidation = [
    (0, express_validator_1.body)('paymentMethodId').isString().isLength({ min: 1, max: 100 }),
    (0, express_validator_1.body)('planId').isString().isLength({ min: 1, max: 100 }),
    (0, express_validator_1.body)('planName').isString().isLength({ min: 1, max: 50 }),
    (0, express_validator_1.body)('userId').optional().isString().isLength({ min: 1, max: 50 })
];
// Create a subscription
app.post('/api/create-subscription', subscriptionValidation, handleValidationErrors, async (req, res) => {
    if (!stripe) {
...
const adminLoginHandler = async (req, res) => {
    ...
    try {
        const user = await prisma_1.prisma.userProfile.findUnique({ where: { email } });
        console.log('🔐 Backend: User found:', !!user, 'isAdmin:', user === null || user === void 0 ? void 0 : user.isAdmin);
...
app.get('/api/admin/me', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        const user = req.user;
        console.log('🔐 Backend: User from token:', user ? 'YES' : 'NO', 'isAdmin:', user === null || user === void 0 ? void 0 : user.isAdmin);
        if (!user || !user.isAdmin) {
...
app.get('/api/admin/users', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { search = '', status = 'all', role = 'all', sort = 'createdAt-desc', page = '1', limit = '50' } = req.query;
...
app.patch('/api/admin/users/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const user = await prisma_1.prisma.userProfile.update({ where: { id }, data: { name, email } });
        res.json(user);
    }
    catch (error) {
...
app.post('/api/admin/users/:id/activate', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await prisma_1.prisma.auditLog.create({
            data: {
                adminUserId: ((_a = req.user) === null || _a === void 0 ? void 0 : _a.id) || '',
...
app.post('/api/admin/users/:id/reset-password', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const user = await prisma_1.prisma.userProfile.update({ where: { id }, data: { password: hashed } });
...
app.post('/api/admin/users/:id/role', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const user = await prisma_1.prisma.userProfile.update({ where: { id }, data: { isAdmin } });
        res.json(user);
    }
    catch (error) {
...
app.get('/api/admin/analytics', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const activeUsers = await prisma_1.prisma.userProfile.count({
            where: {
                workoutLogs: {
                    some: {
                        date: {
        ...
        const newSignups = await prisma_1.prisma.userProfile.count({
            where: {
                createdAt: {
                    gte: sevenDaysAgo
                }
        ...
        const subscriptionBreakdown = await prisma_1.prisma.userProfile.groupBy({
            by: ['tier'],
            _count: {
                tier: true
            }
        ...
        const completedWorkouts = await prisma_1.prisma.workoutLog.count({
            where: {
                completed: true,
                date: {
                    gte: startDate,
        ...
        const totalWorkoutAttempts = await prisma_1.prisma.workoutLog.count({
            where: {
                date: {
                    gte: startDate,
                    lte: endDate
...
app.get('/api/admin/parq', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const flagged = await prisma_1.prisma.userProfile.findMany({ where: { parqCompleted: false } });
        res.json(flagged);
    }
    catch (error) {
...
const registrationValidation = [
    ...
    (0, express_validator_1.body)('name').isLength({ min: 2, max: 50 }).trim(),
    (0, express_validator_1.body)('height').isFloat({ min: 36, max: 96 }), // Height in inches (3 feet to 8 feet)
    (0, express_validator_1.body)('weight').isFloat({ min: 30, max: 500 }),
    (0, express_validator_1.body)('age').isInt({ min: 13, max: 120 })
];
const registrationHandler = async (req, res) => {
    const { email, password, name, height, weight, age, gender, fitnessGoals, activityLevel, equipmentAvailability, preferredWorkoutDuration, dietaryPreferences } = req.body;
    ...
    try {
        const existing = await prisma_1.prisma.userProfile.findUnique({ where: { email } });
        if (existing) {
        ...
        const token = (0, auth_1.generateToken)({ id: user.id, email: user.email, type: 'registered' });
        res.json({
            token,
            user: {
                id: user.id,
...
app.post('/api/register', authLimiter, registrationValidation, handleValidationErrors, registrationHandler);
// User login endpoint
...
app.post('/api/login', authLimiter, loginValidation, handleValidationErrors, loginHandler);
// Logout endpoint
app.post('/api/logout', auth_1.authenticate, async (req, res) => {
    try {
...
app.post('/api/report-error', (0, express_rate_limit_1.default)({
    ...
    try {
        ...
        logger_1.logger.error('Client error reported', {
            message,
            stack: stack === null || stack === void 0 ? void 0 : stack.substring(0, 1000), // Truncate for logging
            userAgent,
            url,
            timestamp,
            severity,
        ...
        try {
            await prisma_1.prisma.errorReport.create({
                data: {
                    message: message.substring(0, 1000), // Ensure we don't exceed DB limits
                    stack: stack === null || stack === void 0 ? void 0 : stack.substring(0, 5000),
                    userAgent: userAgent === null || userAgent === void 0 ? void 0 : userAgent.substring(0, 500),
                    url: url === null || url === void 0 ? void 0 : url.substring(0, 500),
            ...
            logger_1.logger.error('Failed to store error report in database', dbError);
        }
        res.status(201).json({
        ...
        logger_1.logger.error('Error reporting endpoint failed', error);
...
const profileValidation = [
    (0, express_validator_1.body)('name').optional().isLength({ min: 2, max: 50 }).trim(),
    (0, express_validator_1.body)('height').optional().isFloat({ min: 36, max: 96 }), // Height in inches (3 feet to 8 feet)
    (0, express_validator_1.body)('weight').optional().isFloat({ min: 30, max: 500 }),
    (0, express_validator_1.body)('age').optional().isInt({ min: 13, max: 120 }),
    ...
    (0, express_validator_1.body)('activityLevel').optional().isIn(['sedentary', 'light', 'moderate', 'active', 'very_active']),
    (0, express_validator_1.body)('dietaryPreferences').optional().isArray(),
    (0, express_validator_1.body)('equipmentAvailability').optional().isArray(),
    (0, express_validator_1.body)('preferredWorkoutDuration').optional().isLength({ max: 50 })
];
app.put('/api/profile', auth_1.authenticate, profileValidation, handleValidationErrors, async (req, res) => {
    var _a, _b, _c;
    try {
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const { name, height, weight, age, gender, fitnessGoals, activityLevel, dietaryPreferences, equipmentAvailability, preferredWorkoutDuration, notifications, avatar } = req.body;
        // Update user profile
    ...
    try {
        const log = await prisma_1.prisma.workoutLog.create({
            data: {
                ...
                exercises,
                notes,
                completed: completed !== null && completed !== void 0 ? completed : true, // Default to true if not specified
                rating: rating || null,
...
app.get('/api/workout-log', auth_1.authenticate, async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
    try {
        const logs = await prisma_1.prisma.workoutLog.findMany({
            where: { userId },
            orderBy: { date: 'desc' }
        });
        res.json(logs);
...
app.post('/api/generate-workout-names', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const workoutNames = await openaiService_1.openaiService.generateWorkoutNames(planGoal, numberOfWorkouts, validWorkoutTypes);
        res.json({
            success: true,
            workoutNames,
            metadata: {
...
app.post('/api/nutrition-log', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const existingLog = await prisma_1.prisma.nutritionLog.findFirst({
            where: {
                userId,
                date: {
        ...
        if (existingLog) {
            ...
            const mergedFoods = [...existingLog.foods, ...foods];
            const totalCalories = mergedFoods.reduce((sum, food) => sum + (food.calories || 0), 0);
            log = await prisma_1.prisma.nutritionLog.update({
            ...
            // Create new log
            log = await prisma_1.prisma.nutritionLog.create({
                data: {
                    userId,
                    date: new Date(date),
                    foods,
...
app.delete('/api/nutrition-log/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await prisma_1.prisma.nutritionLog.delete({ where: { id } });
        res.status(204).end();
    }
    catch (error) {
...
app.get('/api/nutrition-log/daily/:date', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const log = await prisma_1.prisma.nutritionLog.findFirst({
            where: {
                userId,
                date: {
                    gte: new Date(new Date(date).setHours(0, 0, 0, 0)),
...
app.get('/api/nutrition-log/weekly', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dayLog = logs.find(log => log.date.toISOString().split('T')[0] === dateStr);
            if (dayLog) {
...
app.get('/api/nutrition-log/monthly', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
...
app.get('/api/nutrition-log/insights', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const user = req.user;
        ...
        const recentDaysWithData = recentLogs.filter(log => { var _a; return ((_a = log.foods) === null || _a === void 0 ? void 0 : _a.length) > 0; }).length;
...
app.get('/api/admin/content', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const items = await prisma_1.prisma.contentItem.findMany({
            orderBy: { createdAt: 'desc' }
        });
        res.json(items);
    }
...
app.post('/api/admin/content', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        const item = await prisma_1.prisma.contentItem.create({
...
app.put('/api/admin/content/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    const { title, type, body } = req.body;
    // Validation
    if (!title || !type || !body) {
    ...
    try {
        const item = await prisma_1.prisma.contentItem.update({
            where: { id },
            data: { title, type, body }
        });
        res.json(item);
...
app.delete('/api/admin/content/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    try {
...
const foodScanRoutes_1 = __importDefault(require("./routes/foodScanRoutes"));
const stripeWebhookRoutes_1 = __importDefault(require("./routes/stripeWebhookRoutes"));
...
app.use('/api/stripe', stripeWebhookRoutes_1.default);
app.use('/api/telegram', telegramRoutes_1.default);
app.use('/api/food', foodScanRoutes_1.default);
...
app.post('/api/guest-register', async (req, res) => {
    try {
        ...
        const token = (0, auth_1.generateToken)({ id: user.id, email: user.email, type: 'guest' });
...
app.post('/api/admin/subscriptions/:id/cancel', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const sub = await prisma_1.prisma.subscription.update({ where: { id }, data: { status: 'cancelled', endDate: new Date() } });
        res.json(sub);
    }
    catch (error) {
...
app.post('/api/admin/subscriptions/:id/refund', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const refundPayment = await prisma_1.prisma.payment.create({
            data: {
        ...
        console.log(`Admin refund initiated:`, {
            adminUser: (_a = req.user) === null || _a === void 0 ? void 0 : _a.email,
            subscriptionId: id,
...
app.post('/api/admin/subscriptions/:id/change-plan', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const sub = await prisma_1.prisma.subscription.update({ where: { id }, data: { planId, plan } });
        res.json(sub);
    }
    catch (error) {
...
app.post('/api/admin/plans', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const plan = await prisma_1.prisma.plan.create({ data: { name, price } });
        res.json(plan);
    }
    catch (error) {
...
app.patch('/api/admin/plans/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, price } = req.body;
        const plan = await prisma_1.prisma.plan.update({ where: { id }, data: { name, price } });
        res.json(plan);
    }
    catch (error) {
...
app.get('/api/admin/parq-responses', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const responses = await prisma_1.prisma.parqResponse.findMany({
            include: { user: true },
            orderBy: { createdAt: 'desc' },
        });
        res.json(responses);
...
app.get('/api/admin/parq-responses/:userId', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const response = await prisma_1.prisma.parqResponse.findFirst({ where: { userId }, orderBy: { createdAt: 'desc' } });
        res.json(response);
    }
    catch (error) {
...
app.post('/api/admin/parq-responses/:userId/flag', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const response = await prisma_1.prisma.parqResponse.updateMany({
            where: { userId },
...
app.get('/api/admin/parq-report', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const total = await prisma_1.prisma.parqResponse.count();
        const flagged = await prisma_1.prisma.parqResponse.count({ where: { flagged: true } });
...
app.patch('/api/parq-response', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const response = await prisma_1.prisma.parqResponse.create({
        ...
        await prisma_1.prisma.userProfile.update({ where: { id: userId }, data: updateData });
        res.json(response);
    }
    catch (error) {
...
app.get('/api/admin/flagged-content', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const flaggedContent = await prisma_1.prisma.contentItem.findMany({
            where: {
...
app.post('/api/admin/content/:id/approve', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
        const { id } = req.params;
        // Update content status (you could add an approved field if needed)
        const content = await prisma_1.prisma.contentItem.findUnique({ where: { id } });
        if (!content) {
...
app.post('/api/admin/content/:id/reject', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
        const { id } = req.params;
        const { reason } = req.body;
...
app.post('/api/admin/backup', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
        // Log admin action
        await prisma_1.prisma.auditLog.create({
...
app.post('/api/admin/restore', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
...
app.get('/api/admin/errors', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const errorLogs = await prisma_1.prisma.errorReport.findMany({
            where,
...
app.get('/api/admin/notifications', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const notifications = await prisma_1.prisma.notificationQueue.findMany({
            where,
            orderBy: { createdAt: 'desc' },
            take: Number(limit)
        });
...
app.post('/api/admin/notifications', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
        ...
        // Create notification
        const notification = await prisma_1.prisma.notificationQueue.create({
            data: {
                type,
                recipient,
                subject,
...
app.get('/api/gif-files', async (req, res) => {
    try {
...
app.get('/api/admin/exercise-approvals', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
...
app.patch('/api/admin/exercise-approval/:exerciseId', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
        ...
        const exerciseName = exerciseId.split('_').slice(0, -1).join(' ').replace(/\b\w/g, (l) => l.toUpperCase());
        // Update or create exercise record
        const exercise = await prisma_1.prisma.exercise.upsert({
            where: {
                id: exerciseId
            },
            update: {
                ...
                reviewedBy: reviewedBy || ((_a = req.user) === null || _a === void 0 ? void 0 : _a.name) || 'admin',
            ...
            create: {
                ...
                approvalStatus,
                lastReviewedAt: lastReviewedAt ? new Date(lastReviewedAt) : new Date(),
                reviewedBy: reviewedBy || ((_b = req.user) === null || _b === void 0 ? void 0 : _b.name) || 'admin',
                flaggedReason: flaggedReason || null,
                hideGif: approvalStatus === 'hidden',
...
app.post('/api/admin/gif-registry/refresh', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
...
app.get('/api/admin/gif-mappings', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const mappings = await prisma_1.prisma.exercise.findMany({
            where: whereClause,
            select: {
                id: true,
                name: true,
...
app.post('/api/admin/bulk-approve-gifs', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a;
    try {
        ...
        const reviewedBy = ((_a = req.user) === null || _a === void 0 ? void 0 : _a.name) || 'admin';
        const reviewedAt = new Date();
        const updatePromises = exerciseIds.map(async (exerciseId) => {
            return prisma_1.prisma.exercise.upsert({
                where: { id: exerciseId },
                update: {
                    approvalStatus,
                ...
                create: {
                    id: exerciseId,
                    name: exerciseId.split('_').slice(0, -1).join(' ').replace(/\b\w/g, (l) => l.toUpperCase()),
                    description: 'Exercise pending description', // Required field
                    difficulty: 'intermediate', // Required field
                    approvalStatus,
                    lastReviewedAt: reviewedAt,
...
function generateShoppingList(mealPlan) {
    ...
    mealPlan.forEach(day => {
        var _a;
        (_a = day.meals) === null || _a === void 0 ? void 0 : _a.forEach((meal) => {
            var _a;
            (_a = meal.items) === null || _a === void 0 ? void 0 : _a.forEach((item) => {
                if (item.ingredients) {
...
app.get('/api/workout-plans/check-free-generation', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        ...
        const trialStatus = (0, auth_1.getTrialStatus)(user);
        ...
        const hasAccess = (0, auth_1.hasValidSubscription)(user, 'basic');
        return res.json({
...
app.get('/api/wger/exercises', auth_1.authenticate, async (req, res) => {
    try {
        const { muscles, equipment, language = 2, category, limit = 50 } = req.query;
        const params = new URLSearchParams();
        if (muscles)
            params.append('muscles', muscles.toString());
        if (equipment)
            params.append('equipment', equipment.toString());
        if (language)
...
app.get('/api/wger/exercise/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/api/wger/search', auth_1.authenticate, async (req, res) => {
    try {
...
app.get('/api/admin/openai-usage', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const stats = openaiService_1.openaiService.getUsageStats();
        res.json({
            ...stats,
...
app.post('/api/workout-plans/generate', auth_1.authenticate, cache_1.cacheWorkoutGeneration, async (req, res) => {
    try {
        ...
        // Check if user has valid subscription or trial for basic tier (required for workout generation)
        else if ((0, auth_1.hasValidSubscription)(user, 'basic')) {
            ...
            if (user.tier === 'free' && !user.freeWorkoutTrialUsed) {
                const trialActivated = await (0, auth_1.activateFreeWorkoutTrial)(user.id);
                if (trialActivated) {
        ...
        let filteredExercises = await prisma_1.prisma.exercise.findMany({
            where: {
                isActive: true,
                equipment: {
        ...
        if (filteredExercises.length === 0) {
            ...
            filteredExercises = await prisma_1.prisma.exercise.findMany({
                where: {
                    isActive: true,
                    equipment: {
                        has: 'bodyweight'
        ...
        if (filteredExercises.length === 0) {
            ...
            try {
                ...
                const importPromises = exerciseDatabase.map(async (exercise) => {
                    try {
                        return await prisma_1.prisma.exercise.create({
                            data: {
                                name: exercise.name,
        ...
        console.log(`Found ${exercises.length} exercises for user profile:`, {
            ...
            exerciseNames: exercises.map((ex) => ex.name).slice(0, 10)
        });
        ...
        const generatedPlan = await openaiService_1.openaiService.generateWorkoutPlan(userProfile, exercises);
        ...
        try {
            const savedPlan = await prisma_1.prisma.workoutPlan.create({
                data: {
                    userId: req.user.id,
            ...
            console.log('- Weeks count:', Array.isArray(savedPlan.weeks) ? savedPlan.weeks.length : 0);
...
app.get('/api/workout-plans', auth_1.authenticate, async (req, res) => {
    try {
        const plans = await prisma_1.prisma.workoutPlan.findMany({ where: { userId: req.user.id } });
...
app.get('/api/workout-plans/:id', auth_1.authenticate, async (req, res) => {
    try {
        const plan = await prisma_1.prisma.workoutPlan.findFirst({ where: { id: req.params.id, userId: req.user.id } });
        if (!plan)
...
app.post('/api/workout-plans', auth_1.authenticate, async (req, res) => {
    try {
        ...
        console.log('Creating workout plan with data:', {
            ...
            description,
            duration,
            workoutsLength: workouts === null || workouts === void 0 ? void 0 : workouts.length,
            weeksLength: weeks === null || weeks === void 0 ? void 0 : weeks.length,
            hasWeeks: !!weeks,
        ...
        const plan = await prisma_1.prisma.workoutPlan.create({
            data: {
                userId: req.user.id,
                name,
                description,
...
app.patch('/api/workout-plans/:id', auth_1.authenticate, async (req, res) => {
    try {
        const plan = await prisma_1.prisma.workoutPlan.update({
...
app.delete('/api/workout-plans/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const existingPlan = await prisma_1.prisma.workoutPlan.findFirst({
            where: { id: req.params.id, userId: req.user.id }
        });
        if (!existingPlan) {
            return res.status(404).json({ error: 'Workout plan not found or not authorized' });
        }
        await prisma_1.prisma.workoutPlan.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch (error) {
...
app.patch('/api/workout-plans/:planId/workouts/:workoutId/complete', auth_1.authenticate, async (req, res) => {
    try {
        const { planId, workoutId } = req.params;
        const { rating, notes, duration } = req.body;
        const userId = req.user.id;
        // Get the current workout plan
        const plan = await prisma_1.prisma.workoutPlan.findFirst({
        ...
        const progressData = plan.progressData || {
            totalWorkouts: 0,
            completedCount: 0,
            weeklyProgress: {},
        ...
        if (weeks && Array.isArray(weeks)) {
            weeks.forEach((week) => {
                if (week.days && Array.isArray(week.days)) {
                    totalWorkouts += week.days.filter((day) => !day.isRestDay).length;
                }
            });
        }
        ...
        const updatedPlan = await prisma_1.prisma.workoutPlan.update({
            where: { id: planId },
            data: {
                completedWorkouts,
                progressData,
        ...
        await prisma_1.prisma.workoutLog.create({
            data: {
                userId,
                planId,
                workoutId,
...
app.patch('/api/workout-plans/:planId/workouts/:workoutId/uncomplete', auth_1.authenticate, async (req, res) => {
    try {
        const { planId, workoutId } = req.params;
        const userId = req.user.id;
        // Get the current workout plan
...
app.get('/api/workout-plans/:planId/progress', auth_1.authenticate, async (req, res) => {
    try {
...
app.post('/api/exercise-info', auth_1.authenticate, async (req, res) => {
    var _a;
    try {
        ...
        const exerciseInfo = await openaiService_1.openaiService.supplementExerciseInfo(exercisesWithLevel);
        res.json({
        ...
        const fallbackInfo = ((_a = req.body.exercises) === null || _a === void 0 ? void 0 : _a.map((exercise) => ({
...
app.post('/api/workout-adaptation', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const adaptation = await openaiService_1.openaiService.generateSmartWorkoutAdaptation(feedback, currentWorkout);
        res.json(adaptation);
    }
    catch (error) {
...
app.post('/api/exercise-recommendations', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const recommendations = await openaiService_1.openaiService.generateExerciseRecommendations(preferences, equipment || [], targetMuscles || [], excludeExercises || []);
...
app.post('/api/plateau-detection', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const analysis = await openaiService_1.openaiService.detectPlateauAndSuggestChanges(workoutHistory, performanceMetrics);
        res.json(analysis);
    }
    catch (error) {
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        const configValidation = configValidator_1.backendConfigValidator.validateAll();
        // Test database connection
        const dbConnected = await (0, prisma_1.checkDatabaseConnection)();
        // Test Cloudinary connection
        let cloudinaryConnected = false;
        let cloudinaryError = null;
        try {
            if (configValidation.serviceStatus.cloudinary) {
                const cloudinary = (await Promise.resolve().then(() => __importStar(require('./services/cloudinaryService')))).default;
        ...
        try {
            if (configValidation.serviceStatus.openai) {
                ...
                const usageStats = openaiService_1.openaiService.getUsageStats();
...
app.get('/api/admin/monitoring', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
        logger_1.logger.info('Admin accessing monitoring dashboard', {
        ...
        const logs = logger_1.logger.exportLogs(60 * 60 * 1000); // Last hour
        const performanceReport = logger_1.logger.getPerformanceReport();
        const healthReport = logger_1.logger.getHealthReport();
        const errorSummary = logger_1.logger.getErrorSummary(60 * 60 * 1000);
        ...
        logger_1.logger.error('Failed to generate monitoring data', {
...
app.get('/api/admin/performance/:operation?', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a, _b;
    try {
        const { operation } = req.params;
        const { timeRange } = req.query;
        const timeRangeMs = timeRange ? parseInt(timeRange) : 60 * 60 * 1000; // Default 1 hour
        const report = logger_1.logger.getPerformanceReport(operation, timeRangeMs);
        logger_1.logger.info('Admin accessing performance metrics', {
        ...
        logger_1.logger.error('Failed to get performance metrics', {
...
app.get('/api/admin/cache/stats', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const stats = cacheService_1.cacheService.getStats();
        const debugInfo = cacheService_1.cacheService.getDebugInfo();
        res.json({
            stats,
            debugInfo,
            timestamp: new Date().toISOString()
...
// Clear cache entries
app.post('/api/admin/cache/clear', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { tags, confirm } = req.body;
        if (!confirm) {
            return res.status(400).json({ error: 'Confirmation required to clear cache' });
        }
        const clearedCount = cacheService_1.cacheService.clear(tags);
        logger_1.logger.info('Cache cleared by admin', {
...
// Warm up cache
app.post('/api/admin/cache/warmup', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        await cacheService_1.cacheService.warmUp();
        logger_1.logger.info('Cache warm-up triggered by admin', {
...
app.delete('/api/admin/users/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Find user to delete
        const userToDelete = await prisma_1.prisma.userProfile.findUnique({
            where: { id }
        });
        if (!userToDelete) {
            return res.status(404).json({ error: 'User not found' });
...
app.post('/api/admin/2fa/verify', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { token } = req.body;
        const user = req.user;
        const userProfile = user; // Cast to access all user properties
        ...
        logger_1.logger.info('2FA verification successful', {
        ...
        logger_1.logger.error('2FA verification failed', error);
        res.status(500).json({ error: '2FA verification failed' });
    }
});
// Disable 2FA endpoint
app.post('/api/admin/2fa/disable', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        // Verify password
        const userProfile = user; // Cast to access password
        const isPasswordValid = await bcryptjs_1.default.compare(confirmPassword, userProfile.password);
        if (!isPasswordValid) {
        ...
        logger_1.logger.info('2FA disabled by admin', {
...
// Export user data
app.get('/api/admin/export', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        logger_1.logger.info('User data export requested', {
        ...
        if (format === 'csv') {
            ...
            const csvData = users.map(user => `${user.id},${user.email},${user.name},${user.createdAt.toISOString()},${user.type},${user.subscriptionStatus},${user.tier},${user.parqCompleted}`).join('\n');
...
app.get('/api/admin/analytics/export', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const userRegistrations = await prisma_1.prisma.userProfile.groupBy({
            by: ['createdAt'],
            where: dateFilter.gte ? { createdAt: dateFilter } : undefined,
            _count: true
        });
        const subscriptions = await prisma_1.prisma.userProfile.groupBy({
            by: ['subscriptionStatus'],
            _count: true
        });
        const tiers = await prisma_1.prisma.userProfile.groupBy({
            by: ['tier'],
            _count: true
        });
        const analyticsData = {
        ...
        logger_1.logger.info('Analytics data export requested', {
...
// Export audit logs
app.get('/api/admin/audit-logs/export', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        logger_1.logger.info('Audit log export requested', {
...
app.get('/api/admin/exercises', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { search, category, difficulty, muscleGroup, equipment, isCustom, isActive = 'true', page = '1', limit = '50' } = req.query;
        ...
        logger_1.logger.error('Failed to fetch exercises', error);
...
app.get('/api/admin/exercises/categories', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        logger_1.logger.error('Failed to fetch exercise categories', error);
        res.status(500).json({ error: 'Failed to fetch categories' });
    }
});
// POST /api/admin/exercises - Create new exercise
app.post('/api/admin/exercises', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { name, description, category, muscleGroups, equipment, difficulty, instructions, tips, imageUrl, videoUrl } = req.body;
        // Validation
        ...
        const existing = await prisma_1.prisma.exercise.findFirst({
            where: {
        ...
        logger_1.logger.info('Exercise created', {
        ...
        logger_1.logger.error('Failed to create exercise', error);
        res.status(500).json({ error: 'Failed to create exercise' });
    }
});
// GET /api/admin/exercises/:id - Get single exercise
app.get('/api/admin/exercises/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        logger_1.logger.error('Failed to fetch exercise', error);
        res.status(500).json({ error: 'Failed to fetch exercise' });
    }
});
// PUT /api/admin/exercises/:id - Update exercise
app.put('/api/admin/exercises/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, category, muscleGroups, equipment, difficulty, instructions, tips, imageUrl, videoUrl, isActive } = req.body;
        // Check if exercise exists
        const existing = await prisma_1.prisma.exercise.findUnique({
            where: { id }
        });
        if (!existing) {
            return res.status(404).json({ error: 'Exercise not found' });
        ...
        if (name && name !== existing.name) {
            const duplicate = await prisma_1.prisma.exercise.findFirst({
                where: {
                    name: { equals: name, mode: 'insensitive' },
                    id: { not: id },
                    isActive: true
        ...
        if (muscleGroups !== undefined)
            updateData.muscleGroups = Array.isArray(muscleGroups) ? muscleGroups : [muscleGroups];
        if (equipment !== undefined)
            updateData.equipment = Array.isArray(equipment) ? equipment : [equipment];
        if (difficulty !== undefined)
            updateData.difficulty = difficulty;
        if (instructions !== undefined)
            updateData.instructions = Array.isArray(instructions) ? instructions : [instructions];
        if (tips !== undefined)
            updateData.tips = tips ? (Array.isArray(tips) ? tips : [tips]) : [];
        ...
        logger_1.logger.info('Exercise updated', {
        ...
        logger_1.logger.error('Failed to update exercise', error);
        res.status(500).json({ error: 'Failed to update exercise' });
    }
});
// DELETE /api/admin/exercises/:id - Soft delete exercise
app.delete('/api/admin/exercises/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await prisma_1.prisma.exercise.findUnique({
            where: { id }
        ...
        logger_1.logger.info('Exercise deleted', {
        ...
        logger_1.logger.error('Failed to delete exercise', error);
        res.status(500).json({ error: 'Failed to delete exercise' });
    }
});
// POST /api/admin/exercises/import - Import static exercises from database
app.post('/api/admin/exercises/import', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { exerciseDatabase } = require('../data/exerciseDatabase');
        let imported = 0;
        let skipped = 0;
        ...
        logger_1.logger.info('Exercise import completed', {
        ...
        logger_1.logger.error('Failed to import exercises', error);
...
const getFoodImage = (product) => {
    ...
    const categories = ((_a = product.categories) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
    ...
    for (const [key, placeholder] of Object.entries(foodCategoryPlaceholders)) {
        if (categories.includes(key) ||
            ...
            ((_b = product.product_name) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(key))) {
            return placeholder;
        }
    }
...
app.get('/api/food/search', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const foods = (data.products || []).map((product) => {
            ...
            return {
                ...
                servingSize: '100',
                servingUnit: 'g',
                brand: ((_b = (_a = product.brands) === null || _a === void 0 ? void 0 : _a.split(',')[0]) === null || _b === void 0 ? void 0 : _b.trim()) || '',
                image: getFoodImage(product),
                nutriscore: product.nutriscore_grade || null,
                categories: ((_c = product.categories) === null || _c === void 0 ? void 0 : _c.split(',').map((c) => c.trim()).slice(0, 3)) || []
            };
        }).filter((food) => food.name !== 'Unknown Product' && food.calories > 0);
        const result = {
            foods,
            page: pageNum,
...
app.get('/api/food/barcode/:barcode', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        if (!(0, auth_1.hasValidSubscription)(user, 'premium') && !isTestUser(user.email)) {
            return res.status(403).json({
        ...
        const foodEntry = {
            ...
            nutriscore: product.nutriscore_grade || null,
            categories: ((_c = product.categories) === null || _c === void 0 ? void 0 : _c.split(',').map((c) => c.trim()).slice(0, 5)) || [],
            ingredients: product.ingredients_text || '',
            allergens: ((_d = product.allergens_tags) === null || _d === void 0 ? void 0 : _d.map((tag) => tag.replace('en:', ''))) || [],
            additives: ((_e = product.additives_tags) === null || _e === void 0 ? void 0 : _e.map((tag) => tag.replace('en:', ''))) || [],
            packaging: ((_f = product.packaging_tags) === null || _f === void 0 ? void 0 : _f.map((tag) => tag.replace('en:', ''))) || [],
            stores: ((_g = product.stores) === null || _g === void 0 ? void 0 : _g.split(',').map((s) => s.trim())) || [],
            countries: ((_h = product.countries) === null || _h === void 0 ? void 0 : _h.split(',').map((c) => c.trim())) || []
        };
...
app.post('/api/admin/exercise-media/upload', auth_1.authenticate, auth_1.requireAdmin, uploadGifs.array('gifs', 20), async (req, res) => {
    ...
    try {
        ...
        console.log(`Admin ${(_a = req.user) === null || _a === void 0 ? void 0 : _a.email} uploaded ${uploadedCount} GIFs to category: ${category}`);
        res.json({
...
app.patch('/api/admin/exercise-media/approval/:exerciseId', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        console.log(`Exercise approval updated:`, {
            adminUser: (_c = req.user) === null || _c === void 0 ? void 0 : _c.email,
            exerciseId,
...
app.post('/api/admin/exercise-media/bulk-approve', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        console.log(`Bulk exercise approval:`, {
            adminUser: (_b = req.user) === null || _b === void 0 ? void 0 : _b.email,
...
app.post('/api/admin/exercise-media/setup-warmup-directory', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const fs = require('fs').promises;
        const path = require('path');
...
app.get('/api/admin/exercise-media/validate-gif-structure', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const fs = require('fs').promises;
        const path = require('path');
...
app.post('/api/admin/workout-templates', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a;
    try {
        const { name, description, difficulty, duration, targetMuscleGroups, equipment, exercises, isPublic } = req.body;
        ...
        // Get admin user ID
        const adminUserId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
        if (!adminUserId) {
...
app.get('/api/admin/workout-templates', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const templates = workoutPlans.map(plan => {
            ...
            const totalExercises = weeks.reduce((count, week) => {
                if (week.days && Array.isArray(week.days)) {
                    return count + week.days.reduce((dayCount, day) => {
...
app.post('/api/admin/users/:userId/assign-workout', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
...
app.get('/api/admin/users/:userId/workouts', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    var _a;
    try {
        const { userId } = req.params;
        ...
        const assignments = [
            {
                ...
                assignedBy: (_a = req.user) === null || _a === void 0 ? void 0 : _a.email,
                status: 'active'
            }
        ];
...
app.get('/api/admin/dashboard/stats', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const newUsersToday = await prisma_1.prisma.userProfile.count({
            where: {
                createdAt: {
                    gte: today
                }
...
app.get('/api/admin/dashboard/activity', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const activities = [];
...
app.patch('/api/admin/workout-templates/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (updates.duration)
            dbUpdates.estimatedDuration = updates.duration;
        if (updates.targetMuscleGroups)
            dbUpdates.targetMuscleGroups = updates.targetMuscleGroups;
        if (updates.equipment)
            dbUpdates.equipment = updates.equipment;
        if (updates.isPublic !== undefined)
            dbUpdates.isDefault = updates.isPublic;
        // Update workout plan in database
        const updatedPlan = await prisma_1.prisma.workoutPlan.update({
            where: { id },
            data: dbUpdates
...
function extractDatabaseSettings(settings) {
...
app.get('/api/admin/settings', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const dbSettings = await prisma_1.prisma.systemSettings.findFirst();
        ...
        const settings = {
            ...
            appName: (dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.appName) || process.env.APP_NAME || 'FitArchitect',
            appDescription: (dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.appDescription) || 'Your AI-powered fitness companion',
            supportEmail: (dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.supportEmail) || process.env.SUPPORT_EMAIL || 'support@fitarchitect.com',
            maintenanceMode: (_a = dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.maintenanceMode) !== null && _a !== void 0 ? _a : false,
            allowRegistrations: (_b = dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.allowRegistrations) !== null && _b !== void 0 ? _b : true,
            requireEmailVerification: (_c = dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.requireEmailVerification) !== null && _c !== void 0 ? _c : false,
            notificationEmail: (dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.notificationEmail) || process.env.ADMIN_EMAIL || null,
            defaultUserTier: 'free',
            maxFreeUsers: (_d = dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.maxFreeUsers) !== null && _d !== void 0 ? _d : 1000,
            sessionTimeout: (_e = dbSettings === null || dbSettings === void 0 ? void 0 : dbSettings.sessionTimeout) !== null && _e !== void 0 ? _e : 1440, // 24 hours in minutes
            // Feature Toggles
            features: {
...
app.put('/api/admin/settings', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        console.log('Admin settings update requested by:', (_a = req.user) === null || _a === void 0 ? void 0 : _a.email);
        ...
        const currentSettings = await prisma_1.prisma.systemSettings.findFirst();
        let updatedSettings;
        if (currentSettings) {
            // Update existing settings
            updatedSettings = await prisma_1.prisma.systemSettings.update({
            ...
            updatedSettings = await prisma_1.prisma.systemSettings.create({
                data: dbSettings
            });
        }
        ...
        await prisma_1.prisma.auditLog.create({
            data: {
                adminUserId: ((_b = req.user) === null || _b === void 0 ? void 0 : _b.id) || '',
                adminEmail: ((_c = req.user) === null || _c === void 0 ? void 0 : _c.email) || '',
        ...
        console.log(`✅ Settings updated successfully by ${(_d = req.user) === null || _d === void 0 ? void 0 : _d.email}`);
        ...
        try {
            await prisma_1.prisma.auditLog.create({
                data: {
                    adminUserId: ((_e = req.user) === null || _e === void 0 ? void 0 : _e.id) || '',
                    adminEmail: ((_f = req.user) === null || _f === void 0 ? void 0 : _f.email) || '',
...
if (process.env.NODE_ENV !== 'test') {
    ...
    checkPortAvailability(port).then(available => {
        ...
        app.listen(port, '0.0.0.0', () => {
            ...
            (0, prisma_1.checkDatabaseConnection)().then(connected => {
                if (connected) {
