...
export interface WorkoutTemplate {
  ...
  estimatedDuration: number; // workout duration in minutes
...
// Scoring system for template matching
...
export class WorkoutTemplateService {
   ...
   */
  static findBestTemplate(user: UserProfile): WorkoutTemplate {
    logger.info('Finding best workout template', { 
      userId: user.id, 
    ...
    const scores: MatchScore[] = workoutTemplates.map(template => {
      let score = 0;
      const reasons: string[] = [];

      // Fitness goals matching (weight: 30%)
      const goalMatch = user.fitnessGoals?.some(goal => template.fitnessGoals.includes(goal)) || false;
      if (goalMatch) {
        score += 30;
      ...
      const hasRequiredEquipment = template.equipment.every(equip => 
        user.equipmentAvailability?.includes(equip) || equip === 'bodyweight'
      );
    ...
    if (!selectedTemplate) {
      ...
      logger.warn('No template found, using fallback', { userId: user.id });
      return workoutTemplates[0];
    }

    logger.info('Template matched successfully', {
      userId: user.id,
   ...
   */
  private static getDifficultyScore(user: UserProfile, templateDifficulty: string): number {
    ...
    if (userLevel === templateDifficulty) return 1.0;
    
    const levels = ['beginner', 'intermediate', 'advanced'];
    const userIndex = levels.indexOf(userLevel);
    const templateIndex = levels.indexOf(templateDifficulty);
    
    if (Math.abs(userIndex - templateIndex) === 1) return 0.7;
    return 0.3;
  }
   ...
   */
  static convertTemplateToWorkoutPlan(template: WorkoutTemplate): any {
    return {
      id: template.id,
      name: template.name,
